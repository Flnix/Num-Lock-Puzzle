...
// Images path folder
const IMAGES_PATH = PATH + 'images/';
// Sounds path folder
const SOUNDS_PATH = PATH + 'sounds/';
...
if(window.location.protocol=='file:'){
  loadSound = (src, len) => {
    len=len||1;
    const s = {a:[], i:0, len:len, ready:true};
    for(var i = 0; i < len; i++) s.a.push(new Audio(SOUNDS_PATH+src));
    Sound_arr.push(s);
    return s;
  }
...
// else, if you play from host or localhost
  ...
  loadSound = (filename) => {
    ...
    ajax.open("GET", SOUNDS_PATH+filename, true);
...
function newImage(src) {
  ...
  m.src=IMAGES_PATH+src; // load image from the source
